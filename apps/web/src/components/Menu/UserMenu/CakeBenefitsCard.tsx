...
const CakeBenefitsCard: React.FC<React.PropsWithChildren<CakeBenefitsCardProps>> = ({ onDismiss }) => {
  ...
  const {
    ...
    <>
      <Text>
        {t(`%lockedCake% NEBULA (including rewards) are locked in the NEBULA Pool until %lockedEndTime%`, {
  ...
  const {
    ...
    <>
      <Text>
        {t(`iCAKE allows you to participate in the IFO public sales and commit up to %iCake% amount of NEBULA.`, {
        ...
        <>
          ...
          <Message mt="8px" mb="16px" variant="warning">
            <MessageText maxWidth="200px">
              {t(
                'Lock NEBULA to enjoy the benefits of farm yield boosting, participating in IFOs, voting power boosts, and so much more!',
              )}{' '}
              <NextLinkFromReactRouter
